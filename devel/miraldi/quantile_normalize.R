...
load_coverage_data <- function(filenames, bin_size) {
...
export_results <- function(coverage_data, sequence_info, filenames, prefix, suffix){
    for (i in 1:length(filenames)) {
...
raw_data <- load_coverage_data(args$input, args$bin)
coverage_data = raw_data$coverage_data
sequence_info = raw_data$sequence_info

# Extract score data
print("Extract score data")
score_columns = unlist(lapply(seq(1, length(args$input)), paste, "score", sep="_"))
...
if (!is.null(args$reference)){
    ...
    print("Normalize score data between all input samples")
...
export_results(coverage_data, sequence_info, args$input, args$prefix, args$suffix)
